...
export default function AdminDashboard() {
  const {
    ...
    approveLoan,
    clearLoanAsPaid,
    extendLoanDuration30Days,
    sendProximityReminderMessage,
  } = useApp();
  ...
  const totalCapitalDeployed = displaySet.reduce(
    (sum, l) => sum + (l.status !== "Pending Approval" ? l.amount : 0),
    0,
  );
  const totalExpectedInflow = displaySet.reduce(
    (sum, l) => sum + (l.status !== "Pending Approval" ? l.totalRepayable : 0),
    0,
  );
  const netCalculatedProfits = displaySet.reduce(
    (sum, l) => sum + (l.status !== "Pending Approval" ? l.interest : 0),
    0,
  );
    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-900 border border-slate-800 p-4 rounded-2xl">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Track structural cycles, review credit deployment logs, and push
            warnings.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl relative">
          ...
          <h3 className="text-2xl font-black text-white mt-1 font-mono">
            R {totalCapitalDeployed.toLocaleString("en-ZA")}
          </h3>
          <p className="text-[11px] text-slate-500 mt-1 flex items-center gap-1">
            <Wallet className="w-3.5 h-3.5" /> Deployed 30-Day Principal
          </p>
        </div>
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl relative">
          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest block">
          ...
          <h3 className="text-2xl font-black text-amber-400 mt-1 font-mono">
            R {totalExpectedInflow.toLocaleString("en-ZA")}
          </h3>
          <p className="text-[11px] text-slate-500 mt-1 flex items-center gap-1">
            <TrendingUp className="w-3.5 h-3.5" /> Capitalized Target Recovery
          </p>
        </div>
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl relative">
          <span className="text-[10px] text-emerald-400 font-bold uppercase tracking-widest block">
            Net Calculated Yield Profit
          </span>
          <h3 className="text-2xl font-black text-emerald-400 mt-1 font-mono">
            R {netCalculatedProfits.toLocaleString("en-ZA")}
          </h3>
          <p className="text-[11px] text-emerald-600 mt-1 flex items-center gap-1">
            <DollarSign className="w-3.5 h-3.5" /> Derived 40% Operational
            Premium
          </p>
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 overflow-hidden">
        <h3 className="text-sm font-black text-white uppercase tracking-wider mb-4">
          Contract Asset Ledger Workspace
        </h3>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-slate-800 text-slate-500 uppercase tracking-widest text-[9px] font-bold">
                ...
                <th className="pb-3 px-2 text-right">
                  Administrative Execution Router
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-850/60 text-slate-300">
              {displaySet.map((item) => {
                ...
                const isProximityWarning =
                  daysRemaining <= 5 && item.status === "Active";

                return (
                  <tr
                    key={item.id}
                    className={`hover:bg-slate-950/40 transition-colors ${isProximityWarning ? "bg-red-500/[0.03]" : ""}`}
                  >
                    ...
                    <td className="py-3.5 px-2">
                      {item.status === "Pending Approval" ? (
                        ...
                        <div className="flex items-center gap-1">
                          <span
                            className={`font-mono font-bold ${isProximityWarning ? "text-red-400 animate-pulse" : "text-slate-300"}`}
                          >
                            {daysRemaining > 0
                    ...
                    <td className="py-3.5 px-2 text-right space-y-1">
                      {item.status === "Pending Approval" ? (
                        ...
                        <span className="text-slate-500 text-[11px] italic pr-3 block">
                          Closed Profile Contract
                        </span>
                      ) : (
                        <div className="flex justify-end items-center gap-1.5 flex-wrap">
                          {errorLogs[item.id] && (
                            <span className="text-[9px] font-medium text-red-400 block border border-red-950 bg-red-950/20 p-1 rounded">
                              {errorLogs[item.id]}
                            </span>
                          )}
                          ...
                          <button
                            onClick={() =>
                              sendProximityReminderMessage(item.id)
                            }
