    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fadeIn">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-md overflow-hidden relative shadow-2xl">
        <div className="bg-slate-950 px-5 py-4 border-b border-slate-850 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="p-1 text-slate-400 hover:text-white rounded-lg hover:bg-slate-850 transition"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          {role === "client" ? (
            <div className="space-y-4">
              ...
              <div>
                <label className="text-[10px] text-slate-400 uppercase font-bold tracking-wider block mb-1">
                  WhatsApp Cell Connection
                </label>
                <input
                  type="text"
                  required
                  placeholder="e.g. 0790397516"
                  value={phone}
              ...
              <div>
                ...
                >
                  <option value="First-Timer">
                    First-Time User Profile (Max R500 Limit)
                  </option>
                  <option value="Senior">
                    Senior Endorsed Account (Max R10,000 Limit)
                  </option>
                </select>
              </div>
            </div>
            ...
            <div className="bg-amber-500/5 border border-amber-500/10 rounded-xl p-4 text-center space-y-1.5">
              <p className="text-xs text-amber-400 font-medium leading-relaxed">
                Developer simulation override mode active. Clicking authorize
                drops you straight into the operational monitoring tracking
                database.
              </p>
            </div>
          )}
