...
export function AppProvider({ children }) {
  ...
  const createLoanRequest = (user, requestedAmount, description) => {
    ...
    if (requestedAmount > limit) {
      return {
        success: false,
        msg: `Tier restriction: Max borrowing capacity is R${limit}`,
      };
    }

    const interestFee = requestedAmount * 0.4;
  ...
  const approveLoan = (loanId) => {
    ...
    if (targetLoan) {
      ...
      window.open(
        `https://wa.me/${targetWhatsApp}?text=${encodeURIComponent(payloadMessage)}`,
        "_blank",
      );
    }
  };
  ...
  const clearLoanAsPaid = (loanId) => {
    setLoans((prev) =>
      prev.map((l) =>
        l.id === loanId ? { ...l, remainingBalance: 0, status: "Settled" } : l,
      ),
    );
  };
  ...
  const extendLoanDuration30Days = (loanId) => {
    ...
    if (!activeLoan)
      return { success: false, msg: "Transaction vector missing." };
    ...
    setLoans((prev) =>
      prev.map((l) => {
        if (l.id === loanId) {
          ...
          const recurringInterestLayer = l.amount * 0.4;
          return {
            ...l,
  ...
  const sendProximityReminderMessage = (loanId) => {
    ...
    if (targetLoan) {
      ...
      window.open(
        `https://wa.me/${targetWhatsApp}?text=${encodeURIComponent(warningMessage)}`,
        "_blank",
      );
    }
  };
